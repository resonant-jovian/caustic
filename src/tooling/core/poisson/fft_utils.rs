...
pub fn fft_3d_c2c_scratch(
    ...
    // Transpose: scratch[ix * nz * ny + iz * ny + iy] = buf[ix * ny * nz + iy * nz + iz]
    scratch[..nx * ny * nz]
        ...
        .for_each(|(ix, slab)| {
            for iz in 0..nz {
                for iy in 0..ny {
                    slab[iz * ny + iy] = buf[ix * ny * nz + iy * nz + iz];
                }
            }
        });
    ...
    // Transpose back: buf[ix * ny * nz + iy * nz + iz] = scratch[ix * nz * ny + iz * ny + iy]
    buf.par_chunks_mut(ny * nz)
        .enumerate()
        .for_each(|(ix, slab)| {
            for iz in 0..nz {
                for iy in 0..ny {
                    slab[iy * nz + iz] = scratch[ix * nz * ny + iz * ny + iy];
                }
            }
        });
